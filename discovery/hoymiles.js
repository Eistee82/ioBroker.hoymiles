...
function buildInfoRequest() {
    const payload = Buffer.concat([
        Buffer.from([0x10]), encodeVarint(28800),              // offset = 28800
        Buffer.from([0x28]), encodeVarint(Math.floor(Date.now() / 1000)), // time
    ]);
    const crc = crc16(payload);
    const totalLen = 10 + payload.length;
    const header = Buffer.alloc(10);
    header[0] = 0x48; header[1] = 0x4d; // "HM"
    header[2] = 0xa3; header[3] = 0x01; // InfoData request
    header[4] = 0x00; header[5] = 0x01; // flags
    header[6] = (crc >> 8) & 0xff; header[7] = crc & 0xff;
    header[8] = (totalLen >> 8) & 0xff; header[9] = totalLen & 0xff;
...
function detect(ip, device, options, callback) {
    ...
    socket.on('data', chunk => {
        ...
        if (buffer.length >= 10 &&
            buffer[0] === 0x48 && buffer[1] === 0x4d &&
            buffer[2] === 0xa2 && buffer[3] === 0x01) {
            socket.destroy();
    ...
    socket.on('timeout', () => { socket.destroy(); callback(null, false, ip); });
    socket.on('error',   () => { socket.destroy(); callback(null, false, ip); });
